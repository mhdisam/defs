...
const ResultsPage = () => {
  ...
  const handleErrorClick = (type, incorrect) => {
    ...
    setTimeout(() => setHighlightedError(null), 2000);
  ...
  const copyToClipboard = () => {
    ...
    const toggleErrorText = (key, index) => {
      ...
      {[
        ...
        <div key={key} className={`p-4 rounded-xl shadow-md ${bgColor} border-2 ${borderColor}`}>
          ...
          {categorizedErrors[key].map((issue, index) => {
            ...
            const truncatedText = issue.error.length > 30 ? issue.error.slice(0, 30) + "..." : issue.error;
            return (
            <p 
              key={index} 
              ...
              onClick={() => {
                if (isExpanded) {
                  ...
                  toggleErrorText(key, index);
                }
              }}
            >
              <span className={`${bgerror || "bg-red-400"} px-2 py-1 rounded`}>
                {isExpanded ? issue.error : truncatedText}
              </span>
              <span className="mx-2">→</span>
              <span className="bg-green-500 px-2 py-1 rounded text-center">{issue.fix}</span>
            </p>
            );
          })}
        </div>
                    ...
                    <div
                      ...
                      >
                        <p className="font-semibold">
                          <span className="text-red-500">{issue.error}</span> →{" "}
